...
client = Bot(description="Hyakkimaru is best", command_prefix="hy!", pm_help = True)
client.remove_command('help')


async def status_task():
    while True:
        await client.change_presence(game=discord.Game(name='Use Prefix hy!help'))
        await asyncio.sleep(5)
        await client.change_presence(game=discord.Game(name='with Shiary'))
        await asyncio.sleep(5)
        await client.change_presence(game=discord.Game(name='Terhubung' +str(len(set(client.get_all_members())))+ 'User'))
        await asyncio.sleep(5)
...
async def on_ready():
    ...
    print('--------')
    print('--------')
    print('Started Hyakkimaru')
    print('Created by Shiary')
    client.loop.create_task(status_task())
	
def is_owner(ctx):
    return ctx.message.author.id == "356005708149555202"
...
async def on_member_join(member):
    ...
    embed.set_image(url = 'https://github.com/Shiary/welcome-user/blob/master/20190310_100634.jpg')
    await client.send_message(member,embed=embed)
    print("Sent message to " + member.name)
    channel = discord.utils.get(client.get_all_channels(), server__name='S-ReQue', name='welcome-goodbye')
    r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
...
async def iamdark(ctx):
    author = ctx.message.author
    await client.delete_message(ctx.message)
    role = discord.utils.get(ctx.message.server.roles, name='Utkash Umar')
...
async def registerme(ctx):
    author = ctx.message.author
    r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
    embed = discord.Embed(title="Successfully added", description="S-ReQue role", color = discord.Color((r << 16) + (g << 8) + b))
    embed.set_image(url = 'https://preview.ibb.co/e3iyap/ezgif_3_7dcc4d6bec.gif')
    embed.add_field(name="Enjoy! ", value="Thanks for registering in S-ReQue Team", inline=True)
    
    await client.delete_message(ctx.message)
    role = discord.utils.get(ctx.message.server.roles, name='S-ReQue Team')
    await client.add_roles(ctx.message.author, role)
    print('Added S-ReQue Team role in ' + (ctx.message.author.name))
    await client.send_message(author, embed=embed)
...
async def ownerinfo(ctx):
    embed = discord.Embed(title="Information about owner", description="Bot Name- Hyakkimaru", color=0x00ff00)
    embed.set_footer(text="Copyright@S-ReQue/Shiary")
    embed.set_author(name=" Bot Owner Name- Shiary#5554")
    embed.add_field(name="Site- https://www.youtube.com/channel/UCOQi385w8YwtxB5LP6-Xbpw", value="Thanks for adding our bot", inline=True)
    await client.say(embed=embed)
...
@client.command(pass_context=True)
async def afk(ctx,user:discord.Member):
	await client.say(user.name+ 'You Have Been Afk')
	await asyncio.sleep(120)
	await client.say(user.name+ 'You Are No Longer Afk')
	
@client.command(pass_context=True)
async def hyakki(ctx):
	await client.say('**My Prefix Is** ``hy!``')
	
@client.command(pass_context=True)
async def flipcoin(ctx):
	await client.say('https://tenor.com/view/coin-toss-coin-toss-gif-5017733')	

@client.command(pass_context=True)
async def serverowner(ctx):
	await client.say('https://discord.gg/Ewt5TPA')
	await client.say('Be A Good Member Ok :wink:')
	
@client.command(pass_context=True)
async def Testing(ctx):
	await client.say('Tested Now the member still connected' +str(len(set(client.get_all_members())))+ 'User')
	
@client.command(pass_context = True)
async def kiss(ctx, *, msg = None):
    if '@here' in msg or '@everyone' in msg:
      return
    if not msg: await client.say("Please specify a user to wish")
    await client.say('You Have Been Kissed ' + msg + ' https://tenor.com/o6ur.gif')
    return
...
async def help(ctx):
    ...
    embed.add_field(name = '``Our Help Server Link`` ',value ='https://discord.gg/Ewt5TPA',inline = False)
    embed.add_field(name = 'hy!modhelp ',value ='Explaines all the commands which are only usable by Those who has moderation permissions. Like- Manage Nicknames, Manage Messages, Kick/Ban Members,etc.',inline = False)
    embed.add_field(name = 'hy!generalhelp ',value ='Explaines all the commands which are usable by everyone.',inline = False)
    await client.send_message(author,embed=embed)
...
async def modhelp(ctx):
    ...
    embed.set_image(url = 'https://image.ibb.co/caM2BK/help.gif')
    embed.add_field(name = 'hy!say(Admin permission required) ',value ='Use it like ``hy!say <text>``',inline = False)
    embed.add_field(name = 'hy!embed(Admin permission required) ',value ='Use it like ``hy!embed <text>``',inline = False)
    embed.add_field(name = 'hy!membercount(Kick members Permission Required) ',value ='Use it like ``hy!membercount`` to get membercount',inline = False)
    embed.add_field(name = 'hy!removemod(Admin Permission Required)',value ='Use it like ``hy!removemod @user`` to remove him from mod. Note-You need Moderator role in your server below darkbot to use it.',inline = False)
    embed.add_field(name = 'hy!makemod(Admin Permission Required)',value ='Use it like ``hy!makemod @user`` to make him mod. Note-You need Moderator role in your server below darkbot to use it.',inline = False)
    embed.add_field(name = 'hy!setup(Admin Permission Required)',value ='Use it to add channels, voice channels and roles if your server is not developed currently and you have just 1-2 channels. Note- Use it only 1 time. If you will use same command again then it will do same thing again .i.e It will add true copy of previous channels + true copy of roles that made in previous command use. So be careful.',inline = False)
    embed.add_field(name = 'hy!friend(Admin Permission Required) ',value ='Use it like ``hy!friend @user`` to give anyone Friend of Owner role',inline = False)
    embed.add_field(name = 'hy!role(Manage Roles Permission Required)',value ='Use it like ``hy!role @user <rolename>``.',inline = False)
    embed.add_field(name = 'hy!setnick(Manage nickname permission required)',value ='Use it like ``hy!setnick @user <New nickname>`` to change the nickname of tagged user.',inline = False)
    embed.add_field(name = 'hy!english(Kick members Permission Required)',value ='Use it like ``hy!english @user`` when someone speaks languages other than English.',inline = False)
    embed.add_field(name = 'hy!serverinfo(Kick members Permission Required) ',value ='Use it like ``hy!serverinfo`` to get serverinfo',inline = False)
    embed.add_field(name = 'hy!userinfo(Kick members Permission Required) ',value ='Use it like ``hy!userinfo @user`` to get some basic info of tagged user',inline = False)
    embed.add_field(name = 'hy!kick(Kick members Permission Required)',value ='Use it like ``hy!kick @user`` to kick any user',inline = False)
    embed.add_field(name = 'hy!roles(Kick members Permission Required) ',value ='Use it to check roles present in server',inline = False)
    embed.add_field(name = 'hy!clear(Manage Messages Permission Required)',value ='Use it like ``hy!clear <number>`` to clear any message',inline = False)
    embed.add_field(name = 'hy!mute(Mute members Permission Required)',value ='Use it like ``hy!mute @user <time>`` to mute any user',inline = False)
    embed.add_field(name = 'hy!unmute(Mute members Permission Required) ',value ='Use it like ``hy!unmute @user`` to unmute anyone',inline = False)
    embed.add_field(name = 'hy!ban(Ban members Permission Required) ',value ='Use it like ``hy!ban @user`` to ban any user',inline = False)
    embed.add_field(name = 'hy!rules(Kick members Permission Required)',value ='Use it like ``hy!rules @user <violation type>`` to warn user',inline = False)
    embed.add_field(name = 'hy!warn(Kick members Permission Required)',value ='Use it like ``hy!warn @user <violation type>`` to warn any user',inline = False)    
    embed.add_field(name = 'hy!norole(Kick members Permission Required) ',value ='Use it like ``hy!norole @user`` to warn anyone if he/she asks for promotion',inline = False)
    embed.add_field(name = 'hy!getuser(Kick members Permission Required) ',value ='Use it like ``hy!getuser @rolename`` to get list of all users having a particular role',inline = False)
    await client.send_message(author,embed=embed)
    await client.say('📨 Check DMs For Information')
...
async def generalhelp(ctx):
    ...
    r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
    embed = discord.Embed(color = discord.Color((r << 16) + (g << 8) + b))
    embed.add_field(name = 'hy!poll ',value ='Use it like ``hy!poll "Question" "Option1" "Option2" ..... "Option9"``.',inline = False)
    embed.add_field(name = 'hy!guess ',value ='To play guess game use ``d!guess <number> and number should be between 1-10``',inline = False)
    embed.add_field(name = 'hy!github ',value ='Use it like- ``hy!github uksoftworld/Shiary``',inline = False)
    embed.add_field(name = 'hy!bottutorial ',value ='Use it like ``hy!bottutorial <tutorial name by Shiary>``',inline = False)
    embed.add_field(name = 'hy!dyno ',value ='Use it like ``hy!dyno <dyno command name>``',inline = False)
    embed.add_field(name = 'hy!donate ',value ='Use it to donate us and get a special post on S-ReQue.',inline = False)
    embed.add_field(name = 'hy!ownerinfo ',value ='To get basic information about owner.',inline = False)
    embed.add_field(name = 'hy!sourcecode ',value ='Use it to see darkbot sourcecode.',inline = False)
    embed.add_field(name = 'hy!upvote ',value ='Use it to Upvote our bot and help us to grow',inline = False)
    embed.add_field(name = 'hy!authlink ',value ='Use it to get authorizing link to authorize this bot to your server.',inline = False)
    embed.add_field(name = 'hy!happybirthday @user ',value ='To wish someone happy birthday',inline = False)
    embed.add_field(name = 'hy!technews ',value ='Use it to get tech news',inline = False)
    embed.add_field(name = 'hy!googlefy ',value ='Use it like ``hy!googlefy <string>``.',inline = False)
    embed.add_field(name = 'hy!spacenews ',value ='Use it to get space news',inline = False)
    embed.add_field(name = 'hy!phynews ',value ='Use it to get physycs',inline = False)
    embed.add_field(name = 'hy!verify ',value ='Use it to get verified role. Note- It needs proper setup.',inline = False)
    embed.add_field(name = 'hy!flipcoin ',value ='Flipps coin',inline = False)
    embed.add_field(name = 'hy!rolldice ',value ='Rolls dice',inline = False)
    embed.add_field(name = 'hy!avatar @user ',value ='Shows avatar',inline = False) 
    embed.add_field(name = 'hy!afk',value ='Afk',inline = False)
    embed.add_field(name ='hy!hyakki',value ='To Know His Prefix',inline = False)	
    await client.send_message(author,embed=embed)
    await client.say('📨 Check DMs For Information')
...
@client.command(pass_context = True)
@commands.has_permissions(kick_members=True)
async def Indo(ctx, *, msg = None):
    await client.delete_message(ctx.message)
    if '@here' in msg or '@everyone' in msg:
      return
    if not msg: await client.say("Please specify a user to warn")
    else: await client.say(msg + ' Tolong Untuk Menggunakan Bahasa Indonesia Supaya Staff Atau Member Bisa Mengerti')
    return
    
@client.command(pass_context = True)
async def ping(ctx, *, msg = None):
	ping = (ping * 100)
	await client.say('pong! {ping}ms')
...
async def membercount(ctx, *args):
    ...
    membs_on = str(len([m for m in g.members if not m.status == Status.online]))
    users = str(len([m for m in g.members if not m.bot]))
    users_on = str(len([m for m in g.members if not m.bot and not m.status == Status.online]))
    bots = str(len([m for m in g.members if m.bot]))
    bots_on = str(len([m for m in g.members if m.bot and not m.status == Status.online]))
...
client.run('NTUxNTAwMDU3MDYxMzU5NjI2.D2TvPg.CPboR3Nd7wjAY8fHTjuC6WiMyHw')
